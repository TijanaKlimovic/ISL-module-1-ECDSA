...
class Point(object):
    ...
    def double(self):
        ...
        if self.y == 0:
            ...
            lamda = (3*(self.x ** 2) + self.curve.a) % self.p
            lamda = (lamda * mod_inv(2 * self.y, self.p)) % self.p
            x1 = ((lamda ** 2) - 2 * self.x) % self.p
            y1 = (- (self.y + lamda * (x1 - self.x))) % self.p
            return Point(self.curve, x1, y1)
    ...
    def add(self, other):
        ...
        if type(other) is PointInf:
            ...
            if self.is_equal(other):									#other.x == self.x and other.y == self.y:
        	    return other.double()        								#could be also self.double() however then the reference to other goes bye bye	
            elif other.x == self.x and other.y != self.y and self.curve.is_equal(other.curve):		#😂️ pls appreciate  smiley ty gn
                return PointInf(self.curve)
            elif self.curve.is_equal(other.curve):
                lamda = (self.y - other.y) * mod_inv(self.x - other.x, self.p)
                lamda = lamda % self.p
                x1 = ((lamda ** 2) - self.x - other.x ) % self.p
                y1 = (-(self.y + lamda * (x1 - self.x))) % self.p
                return Point(self.curve, x1, y1)
        else:
            print('Adding points not on the same curve is undefined')
    ...
    def scalar_multiply(self, scalar):
        ...
        #raise NotImplementedError()
        if abs(scalar) % self.curve.q == 0 :	# if absolute value is a multiple of q ie the order of EC we return identity 
            return PointInf(self.curve)
        else: 
            if scalar < 0:
                scalar = abs(scalar) % self.curve.q
                self.y = - self.y		
            else: 
            ...
            binary = bin(scalar)[2:]
            if int(binary[0]) == 0:
                result = self.double() #returns new object
            else: 
...
p = PointInf(nistp256_params.curve)


#print(p.double() is p)
#print(p.add(p) is p)
#x, Q = KeyGen(nistp256_params)
#Sign(nistp256_params, x, msg)
