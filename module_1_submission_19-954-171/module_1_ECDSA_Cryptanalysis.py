...
def recover_x_known_nonce(k, h, r, s, q):
    ...
    x = mod_inv(r, q) * (k * s - h) #inverse of r always exists since the signing algorithm doesnt terminate until s and r are non 0
    x = x % q
    return x
...
def recover_x_repeated_nonce(h_1, r_1, s_1, h_2, r_2, s_2, q):
    ...
    # The function should compute and return the secret signing key x
    x = (h_1 * s_2 - h_2 * s_1) * mod_inv(r_2 * s_1 - r_1 * s_2 , q)
    x = x % q
    return x
...
def MSB_to_Padded_Int(N, L, list_k_MSB):
    ...
    a = 0
    for b in range(L):
        if list_k_MSB[b] == 1:  #may need to have it changed back to an int value depending on the type of elements stored in list 
            a = a + 2 ** (L - 1 - b)  
    k = a * (2 ** (N - L)) + (2 ** (N - L - 1))
    return k        
...
def hnp_to_cvp(N, L, num_Samples, list_t, list_u, q):
    ...
    scale = 2 ** (L + 1)
    for i in range(num_Samples):
        list_of_zeroes = [0] * (num_Samples + 1)  
        list_of_zeroes[i] = q * scale
        matrix_B.append(list_of_zeroes)
        
    t = [i * scale for i in list_t] 
    t.append(1)
    matrix_B.append(t)        
    u = [i * scale for i in list_u]
    u = u + [0]
    return (matrix_B , u)  
...
def cvp_to_svp_Kannan_Embedding(N, L, num_Samples, cvp_basis_B, cvp_list_u):
    ...
    svp_B = []
    scale = 2 ** (L + 1)
    #setting M and scaling it according to basis and u
    scale_M = math.sqrt(num_Samples + 1) 
    low = math.floor(scale_M)
    high = math.ceil(scale_M) 
    if scale_M - low > high - scale_M:
        M = high
    else:
        M = low
    M = scale * M * (2 ** (N - L - 1)) 
...
def recover_x_partial_nonce_SVP(N, L, num_Samples, listoflists_k_MSB, list_h, list_r, list_s, q):
    ...
    # The function should recover the secret signing key x from the output of the SVP solver and output the samed  
    x = -f_List[-2]  
    return x % q
...
def test_single_vector():
    with open('testvectors_hnp_single_sample_256_128.txt', 'r') as filehandle:
        ...
        while line_space != '':
            list_k_MSB = filehandle.readline().split()[2:]
            #print(list_k_MSB)
            line_space = filehandle.readline()
            h = int(filehandle.readline().split()[2])
            r = int(filehandle.readline().split()[2])
            s = int(filehandle.readline().split()[1])
            line_space = filehandle.readline()
            t = int(filehandle.readline().split()[2])
            u = int(filehandle.readline().split()[2])
            line_space = filehandle.readline()
            line_space = filehandle.readline()
            t1, u1 = setup_hnp_single_sample(256, 128, list_k_MSB, h, r, s, q)
            if t1 != t or u1 != u:
                bol = False
                print('t1: ', t1, 't: ', t)
                print('u1: ', u1, 'u: ', u)
            
        print('istina je bolna ', bol)   

def test_all_vector():
     with open('testvectors_hnp_all_samples_256_128_100.txt', 'r') as filehandle:
        bol = True
        line_space = filehandle.readline()  #read the list of lists_k_MSB:
        while line_space != '':
            listoflists_k_MSB = []
            list_h = []
            list_r = []
            ...
            list_u = []
            for i in range(5):
                listoflists_k_MSB.append(filehandle.readline().split())
     
            #print(listoflists_k_MSB)
            line_space = filehandle.readline()
            line_space = filehandle.readline()
            listy = (filehandle.readline().split())
            for i in listy:
                list_h.append(int(i))
            #print(list_h)
            ...
            line_space = filehandle.readline()
            line_space = filehandle.readline()
            #list_r = int(filehandle.readline().split())
            listy = (filehandle.readline().split())
            for i in listy:
                list_r.append(int(i))
            #print(list_r)
            ...
            line_space = filehandle.readline()
            line_space = filehandle.readline()
            #list_s = int(filehandle.readline().split())
            listy = (filehandle.readline().split())
            for i in listy:
                list_s.append(int(i))
            #print(list_s)
            ...
            line_space = filehandle.readline()
            line_space = filehandle.readline()
            #list_t = int(filehandle.readline().split())
            listy = (filehandle.readline().split())
            for i in listy:
                list_t.append(int(i))
            #print(list_t)
            ...
            line_space = filehandle.readline()
            line_space = filehandle.readline()
            #list_u = int(filehandle.readline().split())
            listy = (filehandle.readline().split())
            for i in listy:
                list_u.append(int(i))
            #print(list_u)
            ...
            list_t1, list_u1 = setup_hnp_all_samples(256, 128, 5, listoflists_k_MSB, list_h, list_r, list_s, q)
            ...
            if list_t1 != list_t or list_u != list_u1:
                bol = False
                print('t1: ', list_t1, 't: ', list_t)
                print('u1: ', list_u1, 'u: ', list_u)
...
#recover_x_partial_nonce_SVP(N, L, num_Samples, listoflists_k_MSB, list_h, list_r, list_s, q)
# it just me messin
#test_all_vector()
